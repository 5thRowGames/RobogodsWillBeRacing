...
int GetCellIndex(float3 position, float3 gridBias, float3 gridScale, int3 gridDims)
{
...
#ifndef PROBES
    ,
    /*18*/ __global float4* const       interpNormalsWSBuffer,
    /*19*/ __global unsigned char* const cullingMapBuffer,
    /*20*/ int                          shouldUseCullingMap,
    /*21*/ float                        pushOff,
    /*22*/ int                          superSamplingMultiplier
#endif
...
)
{
    ...
    if (shouldPrepareNewRay && IsInvalidPosition(position))
        shouldPrepareNewRay = false;    // reject texels that are invalid
#ifdef PROBES
...
#endif
    if (shouldPrepareNewRay && IsDirectConverged(currentDirectSampleCount, maxDirectSampleCount))
        shouldPrepareNewRay = false;
...
#else
            PrepareShadowRay(light, sample2D, position.xyz, INDEX_SAFE(interpNormalsWSBuffer, ssIdx).xyz, pushOff, idx, false,&r);
#endif
...
__kernel void prepareLightRaysFromBounce(
    ...
    /*10*/ __global float4*              const pathLastNormalBuffer,
    /*11*/    const int                        lightmapSize,
    /*12*/    const int                        giPassIndex,
    /*13*/    const int                        bounce,
    /*14*/ __global uint* restrict             random_buffer,
    /*15*/ __global uint                const* sobol_buffer,
    /*16*/ float                               pushOff,
    /*17*/ __global uint*               const  activePathCountBuffer_0,
    //*** output ***
    /*18*/ __global ray*                       lightRaysBuffer,
    /*19*/ __global LightSample*               lightSamples,
    /*20*/ __global uint*                      indexRemapBuffer,
    /*21*/ __global uint*                      totalRayCastBuffer,
    /*22*/ __global uint*                      lightRaysCountBuffer
    KERNEL_VALIDATOR_BUFFERS_DEF
)
{
    // Initialize local memory
    ...
    if (shouldPrepareNewRay)
    {
        ...
        else
        {
            ...
            {
                lightSample->lightIdx = INDEX_SAFE(indirectLightsOffsetBuffer, cellIdx) + (int)(sample1D * (float)lightDistribution);
