...
float3 CalculateJitteredLightVec(bool forceHardShadow, LightBuffer light, float3 rayDir, float maxT, float2 sample2D)
{
    ...
    // Construct basis
    float3 u = GetOrthoVector(rayDir);
    float3 v = cross(u, rayDir);
    u = cross(rayDir, v);
...
float SampleFalloff(const int falloffIndex, const float normalizedSamplePosition, __global float* restrict distanceFalloffs_buffer KERNEL_VALIDATOR_BUFFERS_DEF)
{
...
float3 ShadeRectangularAreaLight(
    const LightBuffer       light,
    const ray               surfaceToLightRay
)
{
    const float3 lightDir = surfaceToLightRay.d.xyz;
    const float width = light.dataUnion.areaLightData.areaWidth;
    const float height = light.dataUnion.areaLightData.areaHeight;
...
float LookupAngularFalloffLUT(float angularScale, __global float* const angularFalloffLUT_buffer KERNEL_VALIDATOR_BUFFERS_DEF)
{
...
float3 ShadePointLight(
    ...
    __global float* restrict distanceFalloffs_buffer
...
float3 ShadeSpotLight(
    ...
    __global float*    const angularFalloffLUT_buffer,
    __global float* restrict distanceFalloffs_buffer
    KERNEL_VALIDATOR_BUFFERS_DEF
)
{
