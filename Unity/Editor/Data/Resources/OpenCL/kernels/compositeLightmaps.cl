...
// ----------------------------------------------------------------------------------------------------
uint ConvertLightmapCoordinatesToIndex(int2 lightmapCoords, int lightmapSize)
{
...
__kernel void compositingNormalizeDirectionality(
    ...
    __global float4* const      interpNormalsWSBuffer,
    int2                        tileCoordinates,
    int                         lightmapSize,
    ...
    KERNEL_VALIDATOR_BUFFERS_DEF)
{
    ...
    float3 normalWS = CalculateSuperSampledNormal(index, superSamplingMultiplier, interpNormalsWSBuffer KERNEL_VALIDATOR_BUFFERS);
