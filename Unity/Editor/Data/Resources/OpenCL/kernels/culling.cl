...
)
{
    ...
    else
    {
        ...
        //Normal
        float3 normal = CalculateSuperSampledNormal(idx, superSamplingMultiplier, interpNormalsWSBuffer KERNEL_VALIDATOR_BUFFERS);
...
)
{
    ...
    else
    {
        ...
        int ssIdx = GetSuperSampledIndex(idx, 0, superSamplingMultiplier);
        float4 position = INDEX_SAFE(positionsWSBuffer, ssIdx);
        KERNEL_ASSERT(IsInvalidPosition(position) == false);
